...
def process_input_files(input_path = os.getcwd()+"/" ):
    ...
    for i in range(len(unique_dev_types)):  #Start with Release Types in different columns
        TC_Mod_impact_frame.append(unique_dev_types[i])
    ...
    for i in range(len(release_data)): #Populate the rows in line with the training data (i.e. release level data)
        
        for j in range(len(unique_dev_types)): #Depending upon the release type in each of the release data, appropriate release type column to be marked as 1. Rest all are 0.
            if(release_data.iloc[i,2]==TC_Mod_impact_frame[j]):
        ...
        j=j+1
        counter = 0
        while(j < (len(module_names)+len(unique_dev_types))): #Depending upon the set of modules impacted in each of the release data, appropriate module column to be marked as 1. Rest all are 0.
        ...
        counter_TC_M = len(unique_dev_types) + len(module_names) #Starting index of test case_moddule map columns
    ...
    for j in range(len(unique_dev_types)): #Depending upon the release type of input release, appropriate release type column to be marked as 1. Rest all are 0.
        if(input_release.iloc[0,2]==TC_Mod_impact_frame[j]):
            output_sheet.iloc[i,j] = 1
        else:
            output_sheet.iloc[i,j] = 0
    ...
    output_sheet.to_csv(input_path+"Output_sheet.csv", index=False)
